...
[TestFixture]
internal class GerUnitTextProviderTests
{
    private Mock<ICarPerf> _carpMock = null!;

    private GerUnitTextProvider CreateProvider(Mock<ICarPerf>? carpMock = null)
    {
        return new GerUnitTextProvider(carpMock?.Object ?? _carpMock.Object);
    }
    ...
    public void TopSpeed_Should_Return_Speed_In_KmH_With_Suffix()
    {
        ...
        string expectedKmh = (topSpeedMps * 3.6).ToString("0", CultureInfo.GetCultureInfo("de-DE"));
    ...
    public void TopSpeed_WithDifferentValues_Should_Convert_Correctly()
    {
        ...
        var provider = CreateProvider();
        var result = provider.TopSpeed;
        string expectedKmh = (40.0 * 3.6).ToString("0", CultureInfo.GetCultureInfo("de-DE"));
        Assert.That(result, Is.EqualTo($"{expectedKmh} Km/h"));
    }

    [Test]
    public void Power_Should_Return_Power_In_PS_With_UMin_Format()
    {
        var provider = CreateProvider();
        var result = provider.Power;
        Assert.That(result, Does.Contain("PS bei @"));
        Assert.That(result, Does.Contain("U/min"));
    }

    [Test]
    public void Power_Should_Convert_HP_To_PS()
    {
        var provider = CreateProvider();
        var analysis = new CarpAnalysis(_carpMock.Object);
        var (expectedHp, expectedRpm) = analysis.MaxPower;
        var expectedPs = (expectedHp * 0.9862).ToString("0", CultureInfo.GetCultureInfo("de-DE"));
        var result = provider.Power;
        Assert.That(result, Is.EqualTo($"{expectedPs} PS bei @ {expectedRpm} U/min"));
    }
    ...
    [Test]
    public void Torque_Should_Return_Torque_In_Nm_With_UMin_Format()
    {
        var provider = CreateProvider();
        var result = provider.Torque;
        Assert.That(result, Does.Contain("Nm bei"));
        Assert.That(result, Does.Contain("U/min"));
    }
    ...
    public void Torque_Should_Convert_Correctly()
    {
        ...
        var expectedNm = (expectedTorque * 1.3558179483).ToString("0", CultureInfo.GetCultureInfo("de-DE"));
        var result = provider.Torque;
        Assert.That(result, Is.EqualTo($"{expectedNm} Nm bei {expectedRpm} U/min"));
    }
    ...
    [Test]
    public void MaxRpm_Should_Return_Engine_Max_RPM_With_UMin_Suffix()
    {
        _carpMock.Setup(p => p.EngineMaxRpm).Returns(7000);
        var provider = CreateProvider();
        var result = provider.MaxRpm;
        Assert.That(result, Is.EqualTo("7000 U/min"));
    }
    ...
    public void MaxRpm_WithDifferentValue_Should_Return_Correct_Value()
    {
        ...
        var provider = CreateProvider();
        var result = provider.MaxRpm;
        Assert.That(result, Is.EqualTo("6500 U/min"));
    }
    ...
    public void Gearbox_When_Auto_Equal_Manual_Should_Return_Simple_String()
    {
        ...
        Assert.That(result, Is.EqualTo("4 Gänge"));
    }
    ...
    public void Gearbox_When_Auto_Different_From_Manual_Should_Return_Complex_String()
    {
        ...
        var provider = CreateProvider();
        var result = provider.Gearbox;
        Assert.That(result, Is.EqualTo("4 Gänge (Manuell) / 3 Gänge (Automatik)"));
    }
    ...
    [Test]
    public void Provider_Should_Use_DeDE_Culture()
    {
        _carpMock.Setup(p => p.TopSpeed).Returns(27.777777);  // ~100 km/h
        var provider = CreateProvider();
        var result = provider.TopSpeed;
        // German culture result
    ...
    [Test]
    public void MaxRpm_With_Zero_Value_Should_Return_Zero_With_UMin()
    {
        ...
        var provider = CreateProvider();
        var result = provider.MaxRpm;
        Assert.That(result, Is.EqualTo("0 U/min"));
    }
