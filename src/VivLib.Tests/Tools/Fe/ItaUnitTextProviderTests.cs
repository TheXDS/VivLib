...
[TestFixture]
internal class ItaUnitTextProviderTests
{
    private Mock<ICarPerf> _carpMock = null!;

    private ItaUnitTextProvider CreateProvider(Mock<ICarPerf>? carpMock = null)
    {
        return new ItaUnitTextProvider(carpMock?.Object ?? _carpMock.Object);
    }
    ...
    public void TopSpeed_Should_Return_Speed_In_KmH()
    {
        ...
        string expectedKmh = (topSpeedMps * 3.6).ToString("0", CultureInfo.GetCultureInfo("it-IT"));
    ...
    public void TopSpeed_WithDifferentValues_Should_Convert_Correctly()
    {
        ...
        var provider = CreateProvider();
        var result = provider.TopSpeed;
        string expectedKmh = (40.0 * 3.6).ToString("0", CultureInfo.GetCultureInfo("it-IT"));
        Assert.That(result, Is.EqualTo(expectedKmh));
    }

    [Test]
    public void Power_Should_Return_Power_In_Ch_With_RPM_Separated_By_Slash()
    {
        var provider = CreateProvider();
    ...
    public void Power_Should_Convert_HP_To_Ch()
    {
        ...
        var expectedCh = (expectedHp * 0.9862).ToString("0", CultureInfo.GetCultureInfo("it-IT"));
        var result = provider.Power;
        Assert.That(result, Is.EqualTo($"{expectedCh}/{expectedRpm}"));
    }
    ...
    [Test]
    public void Torque_Should_Return_Torque_In_Lb_Ft_With_RPM_Separated_By_Slash()
    {
        var provider = CreateProvider();
    ...
    [Test]
    public void Torque_Should_Convert_To_Lb_Ft()
    {
        var provider = CreateProvider();
        var analysis = new CarpAnalysis(_carpMock.Object);
        var (expectedTorque, expectedRpm) = analysis.MaxTorque;
        var expectedLbFt = (expectedTorque * 1.3558179483).ToString("0", CultureInfo.GetCultureInfo("it-IT"));
        var result = provider.Torque;
        Assert.That(result, Is.EqualTo($"{expectedLbFt}/{expectedRpm}"));
    }
    ...
    public void Gearbox_When_Auto_Different_From_Manual_Should_Return_Complex_String()
    {
        ...
        var provider = CreateProvider();
        var result = provider.Gearbox;
        Assert.That(result, Is.EqualTo("4 (manuale) / 3 (automatico)"));
    }
    ...
    public void Accel0To60_Should_Return_Estimated_Acceleration_Time()
    {
        ...
        Assert.That(result, Does.Match(@"\d+[,\.]\d"));  // Italian uses comma for decimal separator
    }
    ...
    [Test]
    public void Provider_Should_Use_ItIT_Culture()
    {
        _carpMock.Setup(p => p.TopSpeed).Returns(27.777777);  // ~100 km/h
        var provider = CreateProvider();
        var result = provider.TopSpeed;
        // Italian culture result
