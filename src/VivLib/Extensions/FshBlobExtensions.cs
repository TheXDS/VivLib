...
public static class FshBlobExtensions
{
    ...
    public static Image? ToImage(this FshBlob blob, Color[]? palette = null)
    {
        ...
        // This is for PREVIEW, so it does not matter what pixel format we actually use.
        //                                                         👇
    ...
    public static void ReplaceWith(this FshBlob blob, Image image, Color[]? palette = null)
    {
        if (blob.Magic == FshBlobFormat.Indexed8)
        {
            ...
            var img = image as Image<Rgba32> ?? image.CloneAs<Rgba32>();
            quantizer.BuildPalette(new ExtensivePixelSamplingStrategy(), img);
            palette ??= [.. quantizer.Palette.ToArray().Select(p => new Color(p))];
            using var q = quantizer.QuantizeFrame(img.Frames[0], new Rectangle(0, 0, img.Width, img.Height));
            var p = new List<byte>();
    ...
    // The palette generated by this method is a Web216 standard palette, along with 16 grayscale entries.
